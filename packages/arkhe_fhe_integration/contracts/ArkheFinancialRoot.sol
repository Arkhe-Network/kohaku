...
contract ArkheFinancialRoot {
    ...
    address constant RECENT_ROOT_ADDRESS = address(0); // TBD
    ...
    function verifyAsset(bytes32 sourceId, uint64 slot, bytes32 root) external view returns (bool) {
        ...
        uint64 i = slot % 8192;
        bytes32 entryHash = keccak256(abi.encodePacked(bytes32(0), sourceId, slot, root)); // domain
        bytes32 storageKey = keccak256(abi.encodePacked(bytes32(0), sourceId, i)); // storage domain
        bytes32 stored = bytes32(0); // stub /* consulta a RECENT_ROOT_ADDRESS[storageKey] */
